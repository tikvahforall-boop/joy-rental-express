      ...
      <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        <div className="grid grid-cols-2 gap-8 md:grid-cols-5">
          <div className="col-span-2 md:col-span-1">
            <Link href="/" className="flex items-center gap-2">
              ...
              <span className="text-lg font-bold">Joy Rental Express</span>
            </Link>
          ...
          {Object.entries(FOOTER_LINKS).map(([category, links]) => (
            <div key={category}>
              <h3 className="mb-3 text-sm font-semibold uppercase tracking-wider text-green-300">
                {category}
              </h3>
        ...
        <div className="mt-10 flex flex-col items-center justify-between gap-4 border-t border-green-800 pt-8 sm:flex-row">
          <p className="text-sm text-green-300">
            &copy; {new Date().getFullYear()} Joy Rental Express. All rights
            reserved.
          </p>
...
function SocialPlaceholder({ label }: { label: string }) {
  return (
    <span
      className="flex h-8 w-8 items-center justify-center rounded-full bg-green-800 text-xs text-green-300 transition-colors hover:bg-green-700"
      title={label}
    >
      {label[0]}
    </span>
